...
//            $where['user']=$this->request->param('E_Name');
            $where['E_Mobile']=$this->request->param('E_Mobile');
            if(empty($remark)){
                $this->error('充值理由为空');
            }
            if ($pwd!='cz'.date('Ymd',time())){
                $this->error('密码错误');
            }
            $uid=$mem->where($where)->value('E_UserID');
            if ($uid!=$user){
                $this->error('充值失败');
            }
            $wallet=Db::name('member_wallet')->where(['E_UserID'=>$user])->field('E_Consumption,E_ConsumptionAmout')->find();
            $data['E_UserID']=$user;
            $data['E_CreateDate']=time();
            $data['E_State']=1;
            $data['E_Money']=$money*100;
            $data['E_Type']=1;
            $data['E_Last']=$wallet['E_Consumption']+$money*100;
            $data['E_Abstract']='后台充值，充值者为'.session('name').'充值理由为'.$remark;
            Db::name('member_consumption_bill')->insert($data);
            $wdata['E_ConsumptionAmout']=$wallet['E_ConsumptionAmout']+$money*100;
            $wdata['E_Consumption']=$wallet['E_Consumption']+$money*100;
            Db::name('member_wallet')->where(['E_UserID'=>$user])->update($wdata);
            $this->success('充值成功','member');
        }
        $user=$this->request->param('user');
        if ($user){
            $info=$mem->where(['E_UserID'=>$user])->field('E_UserID,E_Mobile,E_Name,E_TrueName')->find();
            if($info){

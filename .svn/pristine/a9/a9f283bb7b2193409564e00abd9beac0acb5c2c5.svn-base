...
class UlController extends ApiController
{
    ...
    public function Login()
    {
        ...
        $token = cmf_token($res['E_UserCode'].'-'.time(), 'ENCODE', 'qxh');
        $re['data']['token'] = $token;;
...
//        $res['unionid'] = 'FCC5C85BED6E6ABC35419FE368A00727';
        ...
        if ($find['E_UserCode']) {
            ...
            $re['data']['token'] =cmf_token($info['E_UserCode'], 'ENCODE', 'qxh');
        } else {
            $re['msg'] = '授权成功！';
            $re['code'] = '2';
    ...
    public function forgetpwd()
    {
        ...
        if ($result['code'] != 1000) {
            ...
            if ($result === false) {
                ...
                $token = cmf_token($info['E_UserCode'].'-'.time(), 'ENCODE', 'qxh');
                $re['data']['token'] = $token;
                $re['data']['usercode'] = $info['E_UserCode'];
//                session('usercode', $info['E_UserCode']);
//                session('username', $info['E_Mobile']);
